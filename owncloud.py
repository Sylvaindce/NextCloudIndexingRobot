...
class FileInfo(object):
    ...
    def get_size(self):
        if '{http://owncloud.org/ns}size' in self.attributes:
            return int(self.attributes['{http://owncloud.org/ns}size'])
        return None

    def get_fileid(self):
        if '{http://owncloud.org/ns}fileid' in self.attributes:
            return self.attributes['{http://owncloud.org/ns}fileid']
        return None

    def get_oc_permissions(self):
        if '{http://owncloud.org/ns}permissions' in self.attributes:
            return self.attributes['{http://owncloud.org/ns}permissions']
        return None

    def get_owner_display_name(self):
        if '{http://owncloud.org/ns}owner-display-name' in self.attributes:
            return self.attributes['{http://owncloud.org/ns}owner-display-name']
        return None
    ...
    def get_quota_used_bytes(self):
        if '{DAV:}quota-used-bytes' in self.attributes:
            return self.attributes['{DAV:}quota-used-bytes']
        return None
...
class Client(object):
    ...
    def __init__(self, url, **kwargs):
        ...
        ## Disable self-signed certificate ssl warning
        ...
        self._verify_certs = kwargs.get('verify_certs', False)
